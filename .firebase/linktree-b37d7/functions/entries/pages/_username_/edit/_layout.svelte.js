...
const Layout = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  ].join(" ").trim()}" data-svelte-h="svelte-iz4fpu">plan</a> <a href="/login/photo" class="${[
    "step",
    $page.route.id?.includes("photo") || $page.route.id?.includes("bio") || $page.route.id?.includes("edit") ? "step-primary" : ""
  ].join(" ").trim()}" data-svelte-h="svelte-1jrby1u">Photo</a> <a href="${"/" + escape($userData?.username, true) + "/bio"}" class="${[
    "step",
    $page.route.id?.includes("bio") || $page.route.id?.includes("edit") ? "step-primary" : ""
  ].join(" ").trim()}">Bio</a> <a href="${"/" + escape($userData?.username, true) + "/edit"}" class="${["step", $page.route.id?.includes("edit") ? "step-primary" : ""].join(" ").trim()}">Links</a> <a href="${"/" + escape($userData?.username, true)}" class="step">My Profile</a></ul></nav> ${validate_component(AnimatedRoute, "AnimatedRoute").$$render($$result, {}, {}, {
    default: () => {
